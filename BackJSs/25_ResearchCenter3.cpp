...
void selection_spread(int last_idx, vector<Virus> selected, int trial)
{
	...
	{
		queue<Virus> cur_virus_q;
		queue<Virus> next_virus_q;
		bool visited[MAX][MAX] = { 0 };
		bool keep_going = true;

		for (int i = 0; i < selected.size(); i++)
		{
			next_virus_q.push(selected.at(i));
		...
		while (keep_going)
		{
			cur_virus_q = next_virus_q;
			keep_going = false;

			if (g_min <= elapssed_time)
				return;

			while (!cur_virus_q.empty())
			{
				Virus cur_virus = cur_virus_q.front(); cur_virus_q.pop();
			...
			elapssed_time++;
			//cout << "time = " <<elapssed_time << "empty = "<<local_empty_space_num << endl;;
			//print_map(visited);
